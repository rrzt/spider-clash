...
export async function exportClash(nodes, fileName = config.output.clashFileName) {
    ensureDir();
    let proxies = nodes.map(toClashProxy).filter(p => p !== null);
    
    // ==================== 修改开始 ====================
    // 对 proxies 中的 name 进行去重，防止 Clash 校验时报 duplicate name 错误
    const usedNames = new Map();  // key: 已使用的名称, value: true
    for (const p of proxies) {
        let originalName = p.name;
        let newName = originalName;
        let counter = 1;
        // 如果名字已被占用，则追加 _2, _3 ... 直到唯一
        while (usedNames.has(newName)) {
            newName = `${originalName}_${counter}`;
            counter++;
        }
        usedNames.set(newName, true);
        p.name = newName;  // 更新为唯一名称
    }
    // ==================== 修改结束 ====================
